...
class UserMediaImageViewMixin(object):
    ...
    def get_success_url(self):
        ...
        raise Exception(
            "No content object given. Please provide ``next`` in your POST data"
        )
...
class AJAXMultipleImageUploadView(CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        try:
            self.content_object = self.c_type.get_object_for_this_type(pk=self.obj_id)
        except ObjectDoesNotExist:
            raise Http404
        ...
        # function.
        if (
            not hasattr(self.content_object, "user")
            or not self.content_object.user == self.user
        ):
            if not hasattr(
                self.content_object, "user_can_edit"
...
class AJAXSingleImageUploadView(FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if (
            ...
            if not hasattr(
                ...
                raise Http404
        return super(AJAXSingleImageUploadView, self).dispatch(request, *args, **kwargs)
    ...
    def form_valid(self, form):
        ...
        context = {
            "image": image,
            "mode": "single",
            "size": self.request.POST.get("size") or "{}x{}".format(size[0], size[1]),
        }
        # Prepare the json response
        data = {
            "files": [
